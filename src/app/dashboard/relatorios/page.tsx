...
export default function RelatoriosPage() {
  ...
  useEffect(() => {
    filterEntradas();
  }, [entradas, filtroPeriodo, filtroMercado, apenasFavoritas]);
  ...
  function filterEntradas() {
    ...
    if (apenasFavoritas) {
      filtered = filtered.filter((e) => e.favorita === true);
    }
  ...
  const desempenhoPorMercado: DesempenhoMercado[] = entradasFiltradas
    ...
    .sort((a, b) => {
      // Ranking por assertividade (winrate) primeiro
      const byAssert = b.percentGreen - a.percentGreen;
      if (byAssert !== 0) return byAssert;
      // Empate: mais amostras primeiro
      const byTotal = b.total - a.total;
      if (byTotal !== 0) return byTotal;
      // Empate final: ordem alfabética
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <button
                ...
                className={`w-full inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition-colors cursor-pointer border ${
                  apenasFavoritas
                    ? theme === "dark"
